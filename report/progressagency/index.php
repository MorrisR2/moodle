...
$url = new moodle_url('/report/progressagency/index.php', array('course'=>$id));
if ($sort !== '') {
    $url->param('sort', $sort);
}
if ($format !== '') {
...
require_capability('report/progressagency:view',$context);
...
// Ray debug
$userlimitview = '';
if (!empty($USER->profile['Reportrestrictions'])) {
    $userlimitview = '((' . $USER->profile['Reportrestrictions'] . '))';
    $where[] = '((' . $USER->profile['Reportrestrictions'] . '))';
}

// Get user match count
$total = $completion->get_num_tracked_users(implode(' AND ', $where), $where_params, $group); // original
// $total = $completion->get_num_tracked_users(implode(' AND ', $where), $where_params, $group);

// Total user count
$grandtotal = $completion->get_num_tracked_users($userlimitview, array(), $group);
// $grandtotal = $completion->get_num_tracked_users('', array(), $group); // original
...
if ($csv && $grandtotal && count($activities)>0) { // Only show CSV if there are some users/actvs
    ...
    if ($svgcleverness) {
        ...
        $PAGE->requires->js('/report/progressagency/textrotate.js');
    }

    // Handle groups (if enabled)
    groups_print_course_menu($course,$CFG->wwwroot.'/report/progressagency/?course='.$course->id);
}
...
$link = $CFG->wwwroot.'/report/progressagency/?course='.$course->id;
if (strlen($sort)) {
    $link .= '&amp;sort='.$sort;
}
$link .= '&amp;start=';
