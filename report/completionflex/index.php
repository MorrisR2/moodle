 ...
 * Configuration
 */
define('COMPLETIONFLEX_REPORT_PAGE',        25);
define('COMPLETIONFLEX_REPORT_COL_TITLES',  true);
...
$onlyactive = optional_param('onlyactive', false, PARAM_BOOL);

$course = $DB->get_record('course', array('id' => $courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id);

$url = new moodle_url('/report/completionflex/index.php', array('course'=>$course->id));
$PAGE->set_url($url);
...
if (!$completion->has_criteria()) {
    print_error('err_nocriteria', 'completionflex', $CFG->wwwroot.'/course/report.php?id='.$course->id);
}
...
if ($csv) {
    ...
    $PAGE->requires->js('/report/completionflex/textrotate.js');

    // Handle groups (if enabled)
    groups_print_course_menu($course, $CFG->wwwroot.'/report/completionflex/?course='.$course->id);

    global $OUTPUT;
    $select = new single_select(
									new moodle_url(
                                                      "/report/completionflex/index.php",
                                                      array('course'=>$courseid, 'start'=>$start, 'sifirst'=>$sifirst, 'silast'=>$silast)
                                                  ),
									'onlyactive', 
									array(1=>get_string('yes'), 0=>get_string('no')),
                                    $onlyactive, null, "onlyactive" 
								);
	$select->set_label(get_string('onlyactive', 'report_completionflex'));
    echo $OUTPUT->render($select);
    $mform_completionflex_date_form = new completionflex_date_form( null, array() );
    $mform_completionflex_date_form->display();
...
$total = $completion->get_num_tracked_users(implode(' AND ', $where), $where_params, $group, $onlyactive);

// Total user count
$grandtotal = $completion->get_num_tracked_users('', array(), $group, $onlyactive);
...
if ($total) {
    $progress = $completion->get_progress_all(
        ...
        $csv ? 0 : COMPLETIONFLEX_REPORT_PAGE,
        $csv ? 0 : $start,
        $context, 
		$onlyactive
    );
...
$link = $CFG->wwwroot.'/report/completionflex/?course='.$course->id.'&amp;onlyactive='.$onlyactive;
if (strlen($sort)) {
    $link .= '&amp;sort='.$sort;
}
$link .= '&amp;start=';
...
if ($total > COMPLETIONFLEX_REPORT_PAGE) {
    ...
    if ($start > 0) {
        $pstart = max($start - COMPLETIONFLEX_REPORT_PAGE, 0);
    ...
    while ($curstart < $total) {
        ...
        $curstart += COMPLETIONFLEX_REPORT_PAGE;
    }

    // Display next link
    $nstart = $start + COMPLETIONFLEX_REPORT_PAGE;
    if ($nstart < $total) {
...
if (!$csv) {
    ...
    if (COMPLETIONFLEX_REPORT_COL_TITLES) {
