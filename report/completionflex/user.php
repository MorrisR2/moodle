...
require('../../config.php');
require_once($CFG->dirroot.'/report/completionflex/lib.php');
...
if (!report_completionflex_can_access_user_report($user, $course, true)) {
    // this should never happen
    error('Can not access user completion report');
}

add_to_log($course->id, 'course', 'report completionflex', "report/completionflex/user.php?id=$user->id&course=$course->id", $course->id);

$stractivityreport = get_string('activityreport');

$PAGE->set_pagelayout('admin');
$PAGE->set_url('/report/completionflex/user.php', array('id'=>$user->id, 'course'=>$course->id));
...
if (!$rs->valid()) {
    ...
    if ($course->id != 1) {
        $error = get_string('nocompletions', 'report_completionflex'); // TODO: missing string
    } else {
        $error = get_string('nocompletioncoursesenroled', 'report_completionflex'); // TODO: missing string
    }
...
foreach ($courses as $type => $infos) {
    ...
    if (!empty($infos)) {
        ...
        echo '<h1 align="center">'.get_string($type, 'report_completionflex').'</h1>';
        ...
        if ($type === 'complete') {
            echo '<th class="c4 header" scope="col">'.get_string('completiondate', 'report_completionflex').'</th>';
        }
        ...
        foreach ($infos as $c_info) {
            ...
            if (!empty($activities)) {
                ...
                $row = array();
                $row['title'] = get_string('activitiescomplete', 'report_completionflex');
            ...
            foreach ($rows as $row) {
                ...
                switch ($row['status']) {
                    ...
                    case 'No':
                        echo get_string('incomplete', 'report_completionflex');
                        break;
                ...
                echo '</td><td class="c3">';
                if ($first_row) {
                    echo '<a href="'.$CFG->wwwroot.'/blocks/completionstatus/details.php?course='.$c_course->id.'&user='.$user->id.'">'.get_string('detailedview', 'report_completionflex').'</a>';
                }
                echo '</td>';
