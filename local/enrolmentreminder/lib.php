...
function local_enrolmentreminder_cron() {
    ...
    if (! time() > $lastcron) {
        sleep(1); // To ensure that thr same mesage isn't sent next time.
    }
}
...
function local_enrolmentreminder_send_messages($events) {
    ...
    foreach($events as $event) {
        ...
        if (!empty($eventdata->fullmessage)) {
            ...
            add_to_log($course->id, 'enrolmentreminder', 'view:sent message', 'local/enrolmentreminder/?courseid=' . $course->id, '', 0, $user->id);
        }
    }
}
