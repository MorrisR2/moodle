...
/*
 Copyright 2012 Texas Engineering Extension Service
 Based on repository_filesystem copyright 2009, 2010 Dongsheng Cai
*/
...
class repository_teex extends repository {
    ...
    public function __construct($repositoryid, $context = SYSCONTEXTID, $options = array()) {
        ...
        $root = $CFG->dirroot.'/teex/repository/';
        ...
        if (!empty($options['ajax'])) {
            if (!is_dir($this->root_path)) {
                ...
                $ret = array();
                $ret['msg'] = get_string('invalidpath', 'repository_teex');
                $ret['nosearch'] = true;
    ...
    public function get_listing($path = '', $page = '') {
        ...
        $list['path'] = array(
            array('name'=>get_string('root', 'repository_teex'), 'path'=>'')
        );
        $trail = '';
        if (!empty($path)) {
            $parts = explode('/', $path);
    ...
    public static function instance_config_form($mform) {
        ...
        if (has_capability('moodle/site:config', get_system_context())) {
            $path = $CFG->dirroot . '/teex/repository/';
            if (!is_dir($path)) {
                mkdir($path, $CFG->directorypermissions, true);
            }
            if ($handle = opendir($path)) {
                $fieldname = get_string('path', 'repository_teex');
                $choices = array();
                ...
                if (empty($choices)) {
                    $mform->addElement('static', '', '', get_string('nosubdir', 'repository_teex', $path));
                    $mform->addElement('hidden', 'fs_path', '');
                } else {
                    $mform->addElement('select', 'fs_path', $fieldname, $choices);
                    $mform->addElement('static', null, '',  get_string('information','repository_teex', $path));
                }
                closedir($handle);
            }
        } else {
            $mform->addElement('static', null, '',  get_string('nopermissions', 'error', get_string('configplugin', 'repository_teex')));
            return false;
        }
    }
