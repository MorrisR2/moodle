...
// use this function name for course page only
// function completeonvisible_cm_info_view(cm_info $cm) {
// use this function name for all pages
function completeonvisible_cm_info_dynamic(cm_info $cm) {
    global $DB;
    global $CFG;
    global $USER;
    // echo "called completeonvisible_cm_info_dynamic\n";
    // echo "called completeonvisible_cm_info_view\n";
    if ($completeonvisible = $DB->get_record('completeonvisible', array('id'=>$cm->instance), 'id, name, intro, introformat')) {
        ...
        $iframe = ''; // $iframe = "<iframe src=\"$fullurl\" frameborder=0 width=0 height=0></iframe>";
        if ($course = $DB->get_record("course", array("id"=>$cm->course))) {
            // echo "got course record\r\n";
            if (! empty($USER->id) ) {
                // require_login($course, true, $cm);
                // echo "require_login done\r\n";
                if ($cm->uservisible) {
                    ...
                    require_login(); // grade_get_grades requires either require_login or $PAGE->setcontext();
                    $grading_info = grade_get_grades($cm->course, 'mod', 'completeonvisible', $cm->instance, array($USER->id));
                    if (!  ( isset($grading_info) && isset($grading_info->items[0]->grades[$USER->id]->grade) )  ) {
                    */
                    $gradedsql = "SELECT * FROM mdl_grade_grades gg, mdl_grade_items gi WHERE rawgrade > 0 AND gg.itemid=gi.id AND gi.itemtype='mod' AND gi.itemmodule='completeonvisible' AND userid=? AND iteminstance=? AND courseid=?";
                    $is_graded = $DB->count_records_sql($gradedsql, array($USER->id, $cm->instance, $cm->course));
                    if(! $is_graded) {
                        if (!function_exists('grade_get_grades')) { //workaround for buggy PHP versions
                            require_once($CFG->libdir.'/gradelib.php');
                        }
                        ...
                        grade_update('mod/completeonvisible', $cm->course, 'mod', 'completeonvisible', $cm->instance, 0, array($USER->id=>$grade), $params);
                    }
                    ...
                    if ($completion->is_enabled($cm)) {
                        ...
                        if ( $completiondata->completionstate == 0) {
                            $olddebug = $CFG->debugdisplay;
                            $CFG->debugdisplay = 0;
                            // set_module_viewed prints a debug message about the header only because the navigation could be affected. it still works.
                            $completion->set_module_viewed($cm);
                            $CFG->debugdisplay = $olddebug;
                            $completion->update_state($cm, COMPLETION_COMPLETE, $USER->id);
                            // $iframe .= '<script type="text/javascript">setTimeout( function() { document.location.reload(true)}, 1000)</script>';
                        }
                    }
                }
            }
...
function completeonvisible_update_grades($modinstance, $userid, $nullifnone=true) {
    ...
    // echo "lib.php 248" . print_r($modinstance, true);
    // $moduleid = $DB->get_field('modules', 'id', array('name'=>'completeonvisible'));

    $grading_info = grade_get_grades($COURSE->id, 'mod', 'completeonvisible', $modinstance->id, array($userid));
    if ($grading_info) {
        ...
        if (isset($grading_info->items[0]->grades[$userid]->grade) ) {
            ...
            $grade = new stdClass();
            $grade->userid   = $userid;
            if ( ($grade->userid > 0) && ($nullifnone) ) {
                $grade->rawgrade = null;
            } else {
                $grade->rawgrade = rand(1,100);
            }
            $grade->grade = $grade->rawgrade;
            return grade_update('mod/completeonvisible', $COURSE->id, 'mod', 'completeonvisible', $modinstance->id, 0, array($userid=>$grade), $params);
        }
    } else {
        /*
        $moduleid = $DB->get_field('modules', 'id', array('name'=>'completeonvisible'));
        $modinfo = get_fast_modinfo($course);
        $cmid = $DB->get_field('course_modules', 'id', array('course'=>$COURSE->id, 'instance'=>$modinstance->id,'module'=>$modid));
...
function completeonvisible_grade_item_update($modinstance, $grades=NULL) {
    ...
    return grade_update('mod/completeonvisible', $modinstance->courseid, 'mod', 'completeonvisible', $modinstance->id, 0, $grades, $params);
}
...
function completeonvisible_cron () {
    ...
    foreach ($instances as $instance) {
        ...
        // echo "enrolled: " . print_r($enrolledsql, true) . "<br>\r\n";
                 $existinggrades = $DB->get_records_sql("SELECT gg.id, userid FROM {grade_grades} gg, {grade_items} gi
                                            WHERE gg.itemid=gi.id AND itemmodule='completeonvisible' AND finalgrade IS NOT NULL AND iteminstance=?",                 array($instance->id));
        ...
        $enrolled = $DB->get_records_sql($sql, array_merge($enrolledparams, array('iteminstance'=>$instance->id)));
