...
function xmldb_certificate_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2012092102) {
        $table = new xmldb_table('certificate');

        $field = new xmldb_field('tcleosehours', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, 0, 0, 'reportcert');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('template', XMLDB_TYPE_CHAR, '255', XMLDB_UNSIGNED, XMLDB_NOTNULL, 0, 0, 'tcleosehours');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        // Certificate savepoint reached
        upgrade_mod_savepoint(true, 2012092102, 'certificate');
    }
    return true;
}
