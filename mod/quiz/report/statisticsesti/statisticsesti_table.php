 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_statisticsesti_table extends flexible_table {
     ...
     */
    public function statistics_setup($quiz, $cmid, $reporturl, $s, $fromtime = 0, $totime = 0xFFFFFFFF) {
        $this->quiz = $quiz;
        $this->cmid = $cmid;
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'quiz_statisticsesti');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'quiz_statisticsesti');
        }
        */
        ...
        $columns[] = 's';
        $headers[] = get_string('attempts', 'quiz_statisticsesti');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'quiz_statisticsesti');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'quiz_statisticsesti');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'quiz_statisticsesti');

        /*
        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'quiz_statisticsesti');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'quiz_statisticsesti');
        */

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'quiz_statisticsesti');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'quiz_statisticsesti');
    ...
    protected function col_name($question) {
        ...
        if ($question->_stats->subquestion) {
            ...
            $url = new moodle_url($this->baseurl, array('slot' => $question->_stats->slot, 'fromtime_int'=>$this->fromtime, 'totime_int'=>$this->totime));
        }

        if ($url) {
            $name = html_writer::link($url, $name,
                    array('title' => get_string('detailedanalysis', 'quiz_statisticsesti')));
        }
    ...
    protected function col_effective_weight($question) {
        ...
        if ($question->_stats->negcovar) {
            $negcovar = get_string('negcovar', 'quiz_statisticsesti');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'quiz_statisticsesti'),
    ...
    public function  wrap_html_start() {
        ...
        echo $OUTPUT->notification('Note - discrimination statistics are not significant for questions with fewer than 50 attempts');
        if (!$this->is_downloading()) {
