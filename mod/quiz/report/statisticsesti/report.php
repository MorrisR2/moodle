...
require_once($CFG->dirroot . '/mod/quiz/report/statisticsesti/statisticsesti_form.php');
require_once($CFG->dirroot . '/mod/quiz/report/statisticsesti/statisticsesti_table.php');
require_once($CFG->dirroot . '/mod/quiz/report/statisticsesti/statisticsesti_question_table.php');
require_once($CFG->dirroot . '/mod/quiz/report/statisticsesti/qstats.php');
require_once($CFG->dirroot . '/mod/quiz/report/statisticsesti/responseanalysis.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_statisticsesti_report extends quiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        $pageoptions['mode'] = 'statisticsesti';
        $pageoptions['fromtime_int'] = $this->fromtime;
        $pageoptions['totime_int'] = $this->totime;

        $reporturl = new moodle_url('/mod/quiz/report.php', $pageoptions);

        $mform = new quiz_statisticsesti_settings_form($reporturl);
        ...
        $this->table = new quiz_statisticsesti_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'quiz_statisticsesti');
        } else {
            $report = get_string('questionstatsfilename', 'quiz_statisticsesti');
        }
        ...
        $this->table->is_downloading($download, $filename,
                get_string('quizstructureanalysis', 'quiz_statisticsesti'));
        ...
        // echo "report 153 \$questions[1]:\n"; var_dump($questions[1]);
        // Get the data to be displayed.
        list($quizstats, $questions, $subquestions, $s) =
                $this->get_quiz_and_questions_stats($quiz, $currentgroup,
                        $nostudentsingroup, $useallattempts, $groupstudents, $questions, $this->fromtime, $this->totime);
        // echo "report 158 \$questions[1]:\n"; var_dump($questions[1]); exit;
        $quizinfo = $this->get_formatted_quiz_info_data($course, $cm, $quiz, $quizstats);

        // Set up the table, if there is data.
        if ($s) {
            $this->table->statistics_setup($quiz, $cm->id, $reporturl, $s, $this->fromtime, $this->totime);
        }
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && !$groupstudents) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'quiz_statisticsesti'));
                }
            }
        ...
        // echo "report.php 186 \$questions[1]:\n"; var_dump($questions[1]); exit;
        if ($everything) { // Implies is downloading.
            ...
            if ($s) {
                ...
                foreach ($questions as $question) {
                    if (question_bank::get_qtype(
                        ...
                        // print "report.php 203 outputted individual\n";
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                    get_string('backtoquizreport', 'quiz_statisticsesti') . '</a>',
            ...
            echo $OUTPUT->heading(get_string('quizinformation', 'quiz_statisticsesti'));
            ...
            if ($s) {
                echo $OUTPUT->heading(get_string('quizstructureanalysis', 'quiz_statisticsesti'));
                $this->output_quiz_structure_analysis_table($s, $questions, $subquestions);
                $this->output_statistics_graph($quizstats->id, $s);
            }
        }
    ...
    protected function output_individual_question_data($quiz, $question) {
        ...
        $questioninfotable->data[] = array(get_string('questionname', 'quiz_statisticsesti'), $question->name);
        //        $question->name.'&nbsp;'.$datumfromtable['actions']);
        // $questioninfotable->data[] = array(get_string('questiontype', 'quiz_statisticsesti'),
        //        $datumfromtable['icon'] . '&nbsp;' .
        //        question_bank::get_qtype($question->qtype, false)->menu_name() . '&nbsp;' .
        //        $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'quiz_statisticsesti'),
        ...
        $labels = array(
            's' => get_string('attempts', 'quiz_statisticsesti'),
            'facility' => get_string('facility', 'quiz_statisticsesti'),
            'sd' => get_string('standarddeviationq', 'quiz_statisticsesti'),
            'random_guess_score' => get_string('random_guess_score', 'quiz_statisticsesti'),
            'intended_weight' => get_string('intended_weight', 'quiz_statisticsesti'),
            'effective_weight' => get_string('effective_weight', 'quiz_statisticsesti'),
            'discrimination_index' => get_string('discrimination_index', 'quiz_statisticsesti'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'quiz_statisticsesti')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'quiz_statisticsesti'));
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'quiz_statisticsesti'));
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_questiontext_preview_urls($question->questiontext,
                $this->context->id, 'quiz_statisticsesti', $question->id);
    ...
    protected function output_individual_question_response_analysis($question,
        ...
        $qtable = new quiz_statisticsesti_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'quiz_statisticsesti'));
            ...
            if ($this->table->is_downloading() == 'xhtml') {
                $questiontabletitle = get_string('analysisofresponsesfor',
                        'quiz_statisticsesti', $questiontabletitle);
            }
        ...
        $responesstats = new quiz_statisticsesti_response_analyser($question);
        // echo "report.php 408, \$responesstats\r\n"; var_dump($responesstats);
        $responesstats->load_cached($quizstats->id);
        // echo "report.php 411, \$responesstats\r\n"; var_dump($responesstats);
        ...
        foreach ($responesstats->responseclasses as $partid => $partclasses) {
            ...
            $rowdata->part = $partid;
             // echo "report.php 420:\n\r" . print_r($responesstats->responses[$partid], true);
    ...
    protected function get_formatted_quiz_info_data($course, $cm, $quiz, $quizstats) {
        ...
        $todisplay = array('firstattemptscount' => 'number',
                    ...
                    'min' => 'summarks_as_percentage',
                    'max' => 'summarks_as_percentage',
        ...
        $quizinfo = array();
        $quizinfo[get_string('quizname', 'quiz_statisticsesti')] = format_string($quiz->name);
        $quizinfo[get_string('coursename', 'quiz_statisticsesti')] = format_string($course->fullname);
        if ($cm->idnumber) {
        ...
        if ($quiz->timeopen && $quiz->timeclose) {
            $quizinfo[get_string('duration', 'quiz_statisticsesti')] =
        ...
        foreach ($todisplay as $property => $format) {
            ...
            $quizinfo[get_string($property, 'quiz_statisticsesti',
    ...
    protected function download_quiz_info_table($quizinfo) {
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('quizinformation', 'quiz_statisticsesti'));
    ...
    protected function output_statistics_graph($quizstatsid, $s) {
        ...
        $imageurl = new moodle_url('/mod/quiz/report/statisticsesti/statisticsesti_graph.php',
                array('id' => $quizstatsid));
        $graphname = get_string('statisticsestireportgraph', 'quiz_statisticsesti');
    ...
    protected function compute_stats($quizid, $currentgroup, $nostudentsingroup,
            $useallattempts, $groupstudents, $questions, $fromtime = 0, $totime = 0xFFFFFFFF) {
        global $DB;
        ...
        list($fromqa, $whereqa, $qaparams) = quiz_statisticsesti_attempts_sql(
                $quizid, $currentgroup, $groupstudents, true, false, $fromtime, $totime);
        ...
        */
        ...
        list($fromqa, $whereqa, $qaparams) = quiz_statisticsesti_attempts_sql(
                $quizid, $currentgroup, $groupstudents, $useallattempts, false, $fromtime, $totime);


        $sql = "SELECT MIN(quiza.sumgrades) AS min, MAX (quiza.sumgrades) AS max FROM $fromqa WHERE $whereqa";
        $minmax = $DB->get_record_sql($sql, $qaparams, MUST_EXIST);
        $quizstats->min = $minmax->min;
        $quizstats->max = $minmax->max;
        ...
        $qstats = new quiz_statisticsesti_question_stats($questions, $s, $summarksavg);
        $qstats->load_step_data($quizid, $currentgroup, $groupstudents, $useallattempts, $fromtime, $totime);
    ...
    protected function try_loading_cached_stats($quiz, $currentgroup,
            $nostudentsingroup, $useallattempts, $groupstudents, $questions, $fromtime = 0, $totime = 0xFFFFFFFF) {
        global $DB;

        $timemodified = time() - self::TIME_TO_CACHE_STATS;
        $quizstats = $DB->get_record_select('quiz_statisticsesti',
                'quizid = ? AND groupid = ? AND allattempts = ? AND timemodified > ? AND fromtime=? AND totime=?',
                array($quiz->id, $currentgroup, $useallattempts, $timemodified, $fromtime, $totime));
        ...
        $subquestions = array();
        $questionstats = $DB->get_records('quiz_question_statisticsesti',
    ...
    protected function cache_stats($quizid, $currentgroup,
            $quizstats, $questions, $subquestions, $fromtime = 0, $totime = 0xFFFFFFFF) {
        global $DB;
        ...
        // Store the data.
        $quizstats->id = $DB->insert_record('quiz_statisticsesti', $toinsert);

        foreach ($questions as $question) {
            $question->_stats->quizstatisticsid = $quizstats->id;
            $DB->insert_record('quiz_question_statisticsesti', $question->_stats, false);
        }

        foreach ($subquestions as $subquestion) {
            $subquestion->_stats->quizstatisticsid = $quizstats->id;
            $DB->insert_record('quiz_question_statisticsesti', $subquestion->_stats, false);
        }
    ...
    protected function get_quiz_and_questions_stats($quiz, $currentgroup,
        ...
        list($quizstats, $questions, $subquestions, $s) =
                $this->try_loading_cached_stats($quiz, $currentgroup, $nostudentsingroup,
                        $useallattempts, $groupstudents, $questions, $fromtime, $totime);

        if (is_null($quizstats)) {
            // echo "report.php 937, \$quizstats is null\n";
            list($s, $quizstats, $qstats) = $this->compute_stats($quiz->id,
                    $currentgroup, $nostudentsingroup, $useallattempts, $groupstudents, $questions, $fromtime, $totime);

            if ($s) {
                // echo "report.php 942, \$s is not null\n";
                ...
                $this->analyse_responses($quizstatisticsid, $quiz->id, $currentgroup,
                        ...
                        $questions, $subquestions, $fromtime, $totime);
            }
        }
    ...
    protected function analyse_responses($quizstatisticsid, $quizid, $currentgroup,
            $nostudentsingroup, $useallattempts, $groupstudents, $questions, $subquestions, $fromtime = 0, $totime = 0xFFFFFFFF) {

        $qubaids = quiz_statisticsesti_qubaids_condition(
                $quizid, $currentgroup, $groupstudents, $useallattempts, false, $fromtime, $totime);
        // echo "report.php 980: fromtime: $fromtime, totime: $totime, ". print_r($qubaids, true) . "\r\n";
        ...
        foreach ($questions as $question) {
            ...
            // echo "report.php 991: ". print_r($responesstats, true) . "\r\n";
    ...
    protected function everything_download_options() {
        ...
        // $output .= '<input type="hidden" name="fromtime_int" value="'.$this->fromtime.'"/>';
        // $output .= '<input type="hidden" name="totime_int" value="'.$this->totime.'"/>';
        $output .= '<input type="submit" value="' .
                get_string('downloadeverything', 'quiz_statisticsesti') . '"/>';
    ...
    protected function output_caching_info($quizstats, $quizid, $currentgroup,
        ...
        list($fromqa, $whereqa, $qaparams) = quiz_statisticsesti_attempts_sql(
                $quizid, $currentgroup, $groupstudents, $useallattempts, true, false, $fromtime, $totime);
        ...
        $output .= get_string('lastcalculated', 'quiz_statisticsesti', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'quiz_statisticsesti'));
    ...
    protected function clear_cached_data($quizid, $currentgroup, $useallattempts) {
        ...
        $todelete = $DB->get_records_menu('quiz_statisticsesti', array('quizid' => $quizid,
        ...
        $DB->delete_records_select('quiz_question_statisticsesti',
                'quizstatisticsid ' . $todeletesql, $todeleteparams);
        $DB->delete_records_select('quiz_question_response_stats',
                'quizstatisticsid ' . $todeletesql, $todeleteparams);
        $DB->delete_records_select('quiz_statisticsesti',
    ...
    protected function using_attempts_string($useallattempts) {
        if ($useallattempts) {
            return get_string('allattempts', 'quiz_statisticsesti');
        } else {
            return get_string('firstattempts', 'quiz_statisticsesti');
        }
    }
}

function quiz_statisticsesti_attempts_sql($quizid, $currentgroup, $groupstudents,
        $allattempts = true, $includeungraded = false, $fromtime = 0, $totime = 0xFFFFFFFF) {
    global $DB;

    $fromqa = '{quiz_attempts} quiza ';

    $whereqa = 'quiza.quiz = :quizid AND quiza.preview = 0 AND quiza.state = :quizstatefinished';
    $whereqa .= ' AND timefinish > :fromtime AND timefinish < :totime';
    $qaparams = array('quizid' => $quizid, 'quizstatefinished' => quiz_attempt::FINISHED, 'fromtime' => $fromtime, 'totime' => $totime);
 ...
 * {@link quiz_statisticsesti_attempts_sql}
 * @param string $fromqa from quiz_statisticsesti_attempts_sql.
 * @param string $whereqa from quiz_statisticsesti_attempts_sql.
 */
function quiz_statisticsesti_qubaids_condition($quizid, $currentgroup, $groupstudents,
        $allattempts = true, $includeungraded = false, $fromtime = 0, $totime = 0xFFFFFFFF) {
         // echo"1155 fromtime: $fromtime, totime: $totime\r\n";  var_dump(debug_backtrace()); exit;
    list($fromqa, $whereqa, $qaparams) = quiz_statisticsesti_attempts_sql($quizid, $currentgroup,
            $groupstudents, $allattempts, $includeungraded, $fromtime, $totime);
